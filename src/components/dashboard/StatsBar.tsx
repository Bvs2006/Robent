...
export const StatsBar = () => {
  const { tasks, worktrees, workers } = useFleetStore()

  const activeWorkers = workers.filter((w) => w.status === 'running').length || tasks.filter((t) => t.status === 'working').length
  const activeWorktrees = worktrees.filter((wt: any) => wt?.status === 'active').length
  ...
  return (
    <div className="grid grid-cols-4 gap-4 px-6 py-4">
      ...
      <div className="bg-[var(--color-fleet-surface)] border border-[var(--color-fleet-border)] rounded-lg p-4">
        ...
        <div className="text-2xl font-semibold text-[var(--color-fleet-text)] mb-1">{activeWorktrees}</div>
        <div className="text-xs text-green-500">Active</div>
      </div>
      <div className="bg-[var(--color-fleet-surface)] border border-[var(--color-fleet-border)] rounded-lg p-4">
        <h3 className="text-xs uppercase tracking-wider text-[var(--color-fleet-text-muted)] font-medium mb-2">Open Reviews</h3>
        <div className="text-2xl font-semibold text-[var(--color-fleet-text)] mb-1">{openPRs}</div>
        <div className="text-xs text-blue-400">In review</div>
      </div>
      <div className="bg-[var(--color-fleet-surface)] border border-[var(--color-fleet-border)] rounded-lg p-4">
